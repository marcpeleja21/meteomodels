...
export async function fetchMeteoblue(
  ...
  return {
    ...
    hourly: {
      ...
      weathercode:              toNumArr(h1.pictocode).map(mbPictoToWmo),
      windspeed_10m:            toNumArr(h1.windspeed),
      winddirection_10m:        toNumArr(h1.winddirection),
      relative_humidity_2m:     toNumArr(h1.relativehumidity),
      pressure_msl:             toNumArr(h1.sealevelpressure),
      cloudcover:               toNumArr(h1.totalcloudcover),
    },
    daily: {
      time:                        timesDay,
      ...
      weathercode:                 toNumArr(dd.pictocode).map(mbPictoToWmo),
      windspeed_10m_max:           toNumArr(dd.windspeed_max),
      windgusts_10m_max:           toNumArr(dd.windgusts_max ?? dd.windgust_max ?? []),
    },
  }
}
